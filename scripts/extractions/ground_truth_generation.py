...
This script generates THREE types of files for faithfulness evaluation:

1. GROUND TRUTH TABLE:
   - Contains all known information from SHAP values and feature data
   ...
   - Separate tables for credit and law datasets

2. LLM EXTRACTION TEMPLATE:
   - SHAP features: name (filled), rank (EMPTY), sign (EMPTY), value (EMPTY), mentioned (EMPTY)
   - Other features: name (filled), value (EMPTY), mentioned (EMPTY) [includes protected attributes]
   - LLM fills: rank, sign, value, and mentioned for SHAP features; value and mentioned for other features

3. FEATURE DISTRIBUTION REFERENCE:
   - Separate JSON files (distributions_credit.json, distributions_law.json)
   - Contains value distributions for all features across approved instances
   - Format: {"feature_name": {"value": percentage, ...}, ...}
   - Used to validate if LLM correctly extracted distribution information
"""
...
def generate_tables():
    """
    Generate ground truth and template tables for credit and law separately (SHAP only).
    ...
    for dataset_name, config in DATASETS.items():
        ...
        gt_output_path = f'results/ground_truth_{dataset_name}.csv'
        ...
        # Save template table
        template_output_path = f'results/template_{dataset_name}.csv'
        template_df.to_csv(template_output_path, index=False)
        print(f"  ✅ Template saved: {template_output_path}")
...
if __name__ == "__main__":
    ...
    # Display sample rows from credit
    credit_gt = pd.read_csv('results/ground_truth_credit.csv')
    display_sample_rows(credit_gt, 'credit')
    
    print("\n" + "="*80)
    print("FILE STRUCTURE SUMMARY")
    print("="*80)
    print("\n1. Ground Truth Table (ground_truth_*.csv):")
    ...
    print("\n2. Template Table (template_*.csv):")
    print("   ✓ instance_index, original_test_index (filled)")
    print("   ✓ predicted_probability (EMPTY - LLM fills)")
    print("   ✓ SHAP_feature_1-3: name (filled), mentioned (EMPTY), rank (EMPTY), sign (EMPTY), value (EMPTY)")
    print("   ✓ other_feature_X: name (filled), mentioned (EMPTY), value (EMPTY) [includes protected attributes]")
    print("\nNote: Distribution information is now part of the narratives.")
    print("LLM extracts what distribution info was mentioned in each narrative.")
